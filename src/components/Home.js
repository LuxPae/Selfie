...
import {getTimers} from "../API/timer";
import {formatTimestamp} from "../pages/timer/timer";
...
const EventEntry = ({event}) => {
    ...
    const lastsMoreDaysDisplayHour = () => {
        if (event.lastsMoreDays.num === 1) return <h1
            className="text-base font-bold">Dalle {dayjs(event.begin).format("HH:mm")}</h1>
        else if (event.lastsMoreDays.num === event.lastsMoreDays.total) return <h1 className="text-base font-bold">Fino
            alle {dayjs(event.end).format("HH:mm")}</h1>
    }
    ...
    return (
        <div className={`w-screen px-3`}>
            <div
                className={`flex flex-col border-2 ${colors.labelsBorder[event.label]} rounded-lg overflow-hidden mb-4`}
                    ...
                    <div className="flex flex-col items-center">
                        <div className={`flex mb-1 ${colors.labelsTextContrast[event.label]}`}>
                            <div className="flex items-center space-x-2">
                                {event.isTask && <input type="checkbox" checked={false} onChange={handleCompleteTask}
                                                        className="w-4 h-4 cursor-pointer"/>}
                                <span className="font-bold cursor-pointer hover:underline underline-offset-4 w-fit"
                                      onClick={goToEvent}>{truncateLongText(event.title, 50)}</span>
                            </div>
                            {(event.lastsMoreDays && !begin.isAfter(currentDate)) && <span
                                className="translate-y-1 font-normal text-xs">&nbsp;&nbsp;({daysFromBegin}/{event.lastsMoreDays.total})</span>}
                        </div>
                        <div className="flex flex-col justify-center">
                            {(!event.allDay && !event.lastsMoreDays) &&
                                <span className={`text-sm ${text_color}`}>{timeSpan()}</span>}
                        </div>
                    </div>
                </div>
                <span
                    className={`px-3 ${event.description && "py-1"} text-gray-300 self-center text-center overflow-auto`}
                    style={{scrollbarWidth: "none"}}>{truncateLongText(event.description, 100)}</span>
            </div>
        </div>
    )
...
const EventsView = ({tasks}) => {
    ...
    return (
        <div
            className={`${colors.MAIN_BORDER_LIGHT} w-screen h-[620px] h-fit p-6 md:basis-1/2 overflow-auto overscroll-none`}
            style={{scrollbarWidth: "none"}}>
            <div className="flex flex-col justify-center items-center w-full">
                {limitedEvents.length === 0 ?
                    <p className="p-6 text-4xl text-center">Non ci sono {tasks ? "attività" : "eventi"} per i prossimi
                        giorni</p>
                    :
...
export const NoteEntry = ({note}) => {
    ...
    return (
        <div className={`w-screen px-3`}>
            <div className={`flex flex-col border ${colors.MAIN_BORDER_LIGHT} rounded-lg overflow-hidden mb-4`}
                 style={{scrollbarWidth: "none"}}>
                <div className={`py-2 ${colors.MAIN_BG_DARK}`}>
                    <div className="flex flex-col items-center">
                        <div className={`mb-1 ${colors.MAIN_TEXT_LIGHT}`}>
                            <div className="flex items-center space-x-2">
                                <span
                                    className="text-xl font-bold cursor-pointer hover:underline underline-offset-4 w-fit"
                                    onClick={goToNote}>{truncateLongText(note.title, 50)}</span>
                            </div>
                        </div>
                        <div className="flex flex-col justify-center">
                ...
                <div className={`py-2 ${colors.MAIN_BG_DARK}`}>
                    ...
                    <div className="flex flex-col items-center">
                        <div className={`mb-1 ${colors.MAIN_TEXT_LIGHT}`}>
                            <div className="flex items-center space-x-2">
                                <span className="font-bold cursor-pointer hover:underline underline-offset-4 w-fit"
                                      onClick={goToPomodoro}>{formatTimestamp(pomodoro.timestampStart)}</span>
                            </div>
                        </div>
                        <div className="flex flex-col justify-center text-white">
                            <div>Cicli: {pomodoro.cycles}</div>
                            <div>Studio: {pomodoro.studyMinutes}</div>
                            <div>Pausa: {pomodoro.pauseMinutes}</div>
                        </div>
                    </div>
                </div>
                <span className={`px-3 ${pomodoro.comment && "py-1"} text-gray-300 self-center overflow-auto`}
                      style={{scrollbarWidth: "none"}}>{truncateLongText(pomodoro.comment, 50)}</span>
            </div>
        </div>
    )
}
...
const PomodoroView = () => {
    ...
    useEffect(() => {
        getTimers({token: user.token}).then((value) => {
            const timers = value?.data || [];
            console.log(timers);
            setTimers(timers || []);
        });
    }, [user]);

    const sortedPomodoros = timers.sort((a, b) => {
        const update_diff = dayjs(b.timestampStart).diff(dayjs(a.timestampStart))
        const create_diff = dayjs(b.timestampStart).diff(dayjs(a.timestampStart))
        ...
        <>
            ...
            <div className="overflow-auto" style={{scrollbarWidth: "none"}}>
                <div className="w-screen pb-6 border-b-2 flex text-5xl md:text-6xl justify-around md:space-x-8">
                    ...
                    <div
                        className="md:w-full translate-y-[5px] flex items-center justify-center space-x-4 md:space-x-16 text-center">
                        {views.names.map((v, i) => <span key={i} onClick={() => setCurrentViewIndex(i)}
                                                         className={`${v === views.names[currentViewIndex] ? "text-xl md:text-3xl" : "text-xs"}`}>
